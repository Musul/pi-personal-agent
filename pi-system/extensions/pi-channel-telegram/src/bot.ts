...
export type IncomingDocumentHandler = (chatId: number, filePath: string, fileName: string, caption: string | undefined) => void;
...
let onIncomingDocument: IncomingDocumentHandler | null = null;
...
const DOC_DIR = path.join(os.homedir(), ".pi", "agent", "documents");
...
function ensureDocDir(): void {
	try { fs.mkdirSync(DOC_DIR, { recursive: true }); } catch {}
}
	...
	bot.on("message:document", async (ctx) => {
		...
		try {
			...
			const baseName = doc.file_name
				? path.basename(doc.file_name, ext)
				: `doc_${Date.now()}`;
			...
			if (onIncomingDocument) {
				onIncomingDocument(chatId, filepath, doc.file_name || filename, caption);
			} else if (onIncomingMessage) {
				const msg = caption
			...
			console.error("[telebridge] Document download error:", err.message);
		}
	});
...
export function setIncomingDocumentHandler(handler: IncomingDocumentHandler | null): void {
	onIncomingDocument = handler;
}
 ...
 */
export async function sendTextWithRetry(chatId: number, text: string, parseMode?: "HTML", retries = 3): Promise<void> {
	for (let i = 0; i < retries; i++) {
		try {
			await sendText(chatId, text, parseMode);
			return;
		} catch (err: any) {
			if (i === retries - 1) {
				console.error("[telebridge] Send failed after retries:", err.message);
				return;
			}
			const waitMs = 1000 * (i + 1);
			console.log(`[telebridge] Send failed, retrying in ${waitMs}ms...`);
			await delay(waitMs);
		}
	}
}
...
let typingChatId: number | null = null;

/**
 * Start sending typing indicator repeatedly every 4 seconds.
 * Telegram shows the indicator for ~5s per call.
 */
 ...
 */
export async function sendVoice(chatId: number, filePath: string): Promise<void> {
	if (!botInstance) return;
	try {
		await botInstance.api.sendVoice(chatId, new InputFile(filePath));
	} catch (err: any) {
		console.error("[telebridge] Voice send error:", err.message);
	}
}
